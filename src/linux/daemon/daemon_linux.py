...
#         if texto_atual not in clipboard_history:  # Evita duplicados
#             clipboard_history.append(texto_atual)
#             print(f"Novo texto copiado: {texto_atual}")
#         time.sleep(1)  # Checa a cada 1 segundo
#     except KeyboardInterrupt:
#             print("Monitoramento encerrado.")

def get_clipboard_content(history):
    """
    Captura o conteúdo atual do clipboard e atualiza o histórico se for um texto novo.
    :param history: Lista contendo o histórico de textos copiados.
    :return: None
    """
    try:
        texto_atual = pyperclip.paste()
        if texto_atual and texto_atual not in history:
            history.append(texto_atual)
            print(f"Novo texto copiado: {texto_atual}")
    except Exception as e:
